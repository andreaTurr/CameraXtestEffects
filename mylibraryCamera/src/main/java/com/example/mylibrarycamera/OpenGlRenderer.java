...
public final class OpenGlRenderer {
    ...
    public boolean is10BitHdr(@NonNull DynamicRange dynamicRange) {
        return isFullySpecified(dynamicRange) && dynamicRange.getEncoding() != ENCODING_SDR
                && dynamicRange.getBitDepth() == BIT_DEPTH_10_BIT;
    }

    public boolean isFullySpecified(@NonNull DynamicRange dynamicRange) {
        return dynamicRange.getEncoding() != ENCODING_UNSPECIFIED
                && dynamicRange.getEncoding() != ENCODING_HDR_UNSPECIFIED
                && dynamicRange.getBitDepth() != DynamicRange.BIT_DEPTH_UNSPECIFIED;
    }
    /**
    ...
    public void render(long timestampNs, @NonNull float[] textureTransform,
        ...
        if (!EGL14.eglSwapBuffers(mEglDisplay, outputSurface.getEglSurface())) {
            Log.w(TAG, "Failed to swap buffers with EGL error: 0x" + Integer.toHexString(
                    EGL14.eglGetError()));
    ...
    private String getGlExtensionsBeforeInitialized(
        ...
        try {
            ...
            Log.w(TAG, "Failed to get GL extensions: " + e.getMessage(), e);
            return "";
        } finally {
            releaseInternal();
        }
    ...
    private void createEglContext(@NonNull DynamicRange dynamicRange) {
        ...
        int rgbBits = is10BitHdr(dynamicRange) ? 10 : 8;
        int alphaBits = is10BitHdr(dynamicRange) ? 2 : 8;
        int renderType = is10BitHdr(dynamicRange) ? EGLExt.EGL_OPENGL_ES3_BIT_KHR
                : EGL14.EGL_OPENGL_ES2_BIT;
        // recordableAndroid with EGL14.EGL_TRUE causes eglError for 10BitHdr.
        int recordableAndroid = is10BitHdr(dynamicRange) ? EGL14.EGL_FALSE : EGL14.EGL_TRUE;
        ...
        int[] attribToCreateContext = {
                EGL14.EGL_CONTEXT_CLIENT_VERSION, is10BitHdr(dynamicRange) ? 3 : 2,
                EGL14.EGL_NONE
        };
    ...
    private void createProgram(@NonNull DynamicRange dynamicRange,
        ...
        try {
            vertexShader = loadShader(GLES20.GL_VERTEX_SHADER,
                    is10BitHdr(dynamicRange) ? HDR_VERTEX_SHADER : DEFAULT_VERTEX_SHADER);
    ...
    private int loadFragmentShader(@NonNull DynamicRange dynamicRange,
        ...
        if (shaderProvider == ShaderProvider.DEFAULT) {
            return loadShader(GLES20.GL_FRAGMENT_SHADER,
                    is10BitHdr(dynamicRange) ? HDR_FRAGMENT_SHADER : DEFAULT_FRAGMENT_SHADER);
        } else {
    ...
    private OutputSurface getOutSurfaceOrThrow(@NonNull Surface surface) {
        checkState(mOutputSurfaceMap.containsKey(surface),
    ...
    private OutputSurface createOutputSurfaceInternal(@NonNull Surface surface) {
        ...
        try {
            ...
            Log.w(TAG, "Failed to create EGL surface: " + e.getMessage(), e);
            return null;
        }
    ...
    private void removeOutputSurfaceInternal(@NonNull Surface surface, boolean unregister) {
        ...
        if (removedOutputSurface != null && removedOutputSurface != NO_OUTPUT_SURFACE) {
            try {
                ...
                Log.w(TAG, "Failed to destroy EGL surface: " + e.getMessage(), e);
            }
        }
    }
